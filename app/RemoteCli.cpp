            ...
            while (true)
            {
                ...
                if (action == TEXT("s")) { /* status display and device selection */
                    ...
                    while (true) {
                        ...
                        else if (select == TEXT("8")) { /* Exposure/ISO Loop Shot */
                            int shots = 1;
                            int delay = 1000;

                            // 如有需要仍可讓使用者調整 shots 和 delay
                            cli::text input;
                            cli::tout << TEXT("Shots per setting (default = 1): ");
                            std::getline(cli::tin, input);
                            if (!input.empty()) shots = std::stoi(input);

                            cli::tout << TEXT("Delay per shot (ms) (default = 1000): ");
                            std::getline(cli::tin, input);
                            if (!input.empty()) delay = std::stoi(input);

                            camera->run_exposure_iso_loop_by_range(shots, delay);
                        }
